        ...
        init: function() {
            ...
            $(document).on('change', '#enable_stock', function() {
                if (this.checked) {
                    ...
                    $('#quick_product_opening_stock_div').show();
                    //Enable expiry selection
                    if ($('#expiry_period_type').length) {
                        $('#warenty_period_type').removeAttr('disabled');
                    }
                    ...
                    $('#quick_product_opening_stock_div').hide();
            ...
            $(document).on('change', '#is_lifetime', function() {
                if (this.checked) {
                    $('#warenty_row').hide();
                    $('#warenty_period').attr('disabled', true);
                } else {
                    $('div#warenty_row').show();
                    $('#warenty_period').attr('disabled', false);
                }
            });
            //Start For product type single
            //If purchase price exc tax is changed
            $(document).on('change', '#single_dpp', function(e) {
            ...
            $(document).on('change', '#tax', function() {
                if ($('#type').val() == 'single') {
                    ...
                    var purchase_inc_tax = __add_percent(purchase_exc_tax, tax_rate);
                    __write_number($('#single_dpp_inc_tax'), purchase_inc_tax);
                    var selling_price = __read_number($('#single_dsp'));
                    var selling_price_inc_tax = __add_percent(selling_price, tax_rate);
                    __write_number($('#single_dsp_inc_tax'), selling_price_inc_tax);
                }
            });
            //If purchase price inc tax is changed
            $(document).on('change', '#single_dpp_inc_tax', function(e) {
                var purchase_inc_tax = __read_number($('#single_dpp_inc_tax'));
                ...
                __write_number($('#single_dpp'), purchase_exc_tax);
                $('#single_dpp').change();
                var profit_percent = __read_number($('#profit_percent'));
            ...
            $(document).on('change', '#profit_percent', function(e) {
                var tax_rate = $('#tax')
                    .find(':selected')
                    .data('rate');
                tax_rate = tax_rate == undefined ? 0 : tax_rate;
            ...
            $(document).on('change', '#single_dsp', function(e) {
                var tax_rate = $('#tax')
                    .find(':selected')
                    .data('rate');
                tax_rate = tax_rate == undefined ? 0 : tax_rate;
            ...
            $(document).on('change', '#single_dsp_inc_tax', function(e) {
                var tax_rate = $('#tax')
                    .find(':selected')
                    .data('rate');
                tax_rate = tax_rate == undefined ? 0 : tax_rate;
                var selling_price_inc_tax = __read_number($('#single_dsp_inc_tax'));
            ...
            $(document).on('click', '.add_variation_value_row', function() {
                ...
                $.ajax({
                    method: 'GET',
                    url: '/admin/products/get_variation_value_row',
                    data: {
            ...
            $(document).on('change', '.variation_template', function() {
                ...
                if ($(this).val() !== '') {
                    ...
                    $.ajax({
                        method: 'POST',
                        url: '/admin/products/get_variation_template',
                        dataType: 'html',
                        data: {
            ...
            $(document).on('change', '#warenty_period_type', function(e) {
                if ($(this).val()) {
                    $('input#warenty_period').prop('disabled', false);
                } else {
                    $('input#warenty_period').val('');
                    $('input#warenty_period').prop('disabled', true);
                }
            });
