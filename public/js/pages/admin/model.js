...
var DatatableResponsive = function() {
        ...
        tariq = $('.content_management_datatable').DataTable({
            ...
            buttons: [{
                ...
                action: function(e, dt, node, config) {
                    datatableSelectedRowsAction(dt, 'action/models', action = 'delete', msg = Lang.get('service.delete_message'));
                }
            }],
            columns: [{
                defaultContent: '',
    ...
    return {
        init: function() {
            ...
            $(document).on('click', '#add_model_values', function() {
                var html =
                    '<div class="form-group row"><div class="col-sm-7 ml-auto"><input type="text" name="name[]" class="form-control" required></div><div class="col-sm-2"><button type="button" class="btn btn-danger delete_model_value">-</button></div></div>';
                $('#variation_values').append(html);
            });
            $(document).on('click', '.delete_model_value', function() {
                $(this)
                    .closest('.form-group')
                    .remove();
            });
