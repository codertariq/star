<tr class="product_row" data-row_index="{{$row_count}}">
	<td>
		...
		<div class="modal fade row_description_modal" id="row_description_modal_{{$row_count}}" tabindex="-1" role="dialog" data-backdrop="static">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="row">
{{--
							<div class="col-md-12 @if(!$product->enable_sr_no) hide @endif">
								<div class="form-group">
									<label>@lang('lang_v1.select_serial')</label>
									{{ Form::select('products['.$row_count.'][serial_no][]', $product->serial, Null, ['class' => 'form-control select2', 'multiple', 'id' => 'serial_no']) }}
								</div>
							</div> --}}
							<div class="col-md-12">
								<div class="form-group">
	...
	<td>
		...
		<div class="input-group input-number">
			...
			{{-- @endif --}}
			<input type="text" {{-- @if($product->enable_sr_no)readonly @endif --}} data-min="1" class="form-control pos_quantity input_number mousetrap input_quantity" value="{{@format_quantity($product->quantity_ordered)}}" name="products[{{$row_count}}][quantity]" data-allow-overselling="@if(empty($pos_settings['allow_overselling'])){{'false'}}@else{{'true'}}@endif"
		...
		<input type="hidden" name="products[{{$row_count}}][is_lifetime]" value="{{$product->is_lifetime}}">
		<input type="hidden" name="products[{{$row_count}}][warenty_period]" value="{{$product->warenty_period}}">
		<input type="hidden" name="products[{{$row_count}}][warenty_period_type]" value="{{$product->warenty_period_type}}">
...
{{-- <script>
	$('#row_description_modal_{{$row_count}}').on('hidden.bs.modal', function() {
		var parent = $(this)
.parents('tr');
var quantity = parent.find('.pos_quantity').val();
if(quantity < 1){
parent.remove();
}
pos_total_row();
});
$('#row_description_modal_{{$row_count}}')
.find('.select2')
.each(function() {
$(this).select2({
dropdownParent: $('#row_description_modal_{{$row_count}}'),
