...
class OpeningStockController extends Controller {
		...
		if (!empty($product) && $product->enable_stock == 1) {
			...
			foreach ($transactions as $transaction) {
				...
				foreach ($transaction->purchase_lines as $purchase_line) {
					...
					// $purchase_lines[$purchase_line->variation_id][$k]['serial_no'] = $purchase_line->lot_number;
				}
			...
			if (request()->ajax()) {
				return view('admin.opening_stock.ajax_add')
					->with(compact(
						'product',
						'locations',
						'purchases',
			...
			return view('admin.opening_stock.add')
				->with(compact(
					'product',
					'locations',
					'purchases',
	...
	public function save(Request $request) {
				...
				if (array_key_exists($location_id, $locations)) {
					...
					foreach ($value as $k => $rows) {
						foreach ($rows as $key => $v) {
							...
							$serial = $this->productUtil->productSerialNo($v['serial_no'], $qty_remaining, $product->id, $k);
							...
							if (isset($v['purchase_line_id'])) {
								...
								if ($qty_remaining != 0) {
									...
									$this->productUtil->updateProductSerial($product->id, $k, $serial, $v['purchase_line_id']);
								}
							} else {
								if ($qty_remaining != 0) {
					...
					if (!empty($purchase_lines)) {
						...
						ProductSerial::where('sell_line_id', Null)->whereIn('purchase_line_id', $delete_purchase_line_ids)
							->delete();
						if ($new_serial) {
							foreach ($transaction->purchase_lines as $transaction_purchase) {
								$this->productUtil->updateProductSerial($product->id, $k, $serial, $transaction_purchase->id);
							}
						}
		...
		$output = ['message' => __('service.opening_stock_added_successfully'),
		];
		// } catch (\Exception $e) {
		// 	DB::rollBack();
