...
class TaxRateRepositoriy extends Repository {
	public function __construct(TaxRate $model) {
		$this->model = $model;
		$this->permission = '';
		$this->route = 'admin.tax-rates.';
	...
	public function selectAll() {
		return $this->model->all()->pluck('name', 'id')->prepend(__('service.select', ['attribute' => __('page.tax_rates')]), '');
	}
	/**
	...
	public function findOrFail($id, $field = 'message') {
		...
		if (!$model) {
			throw ValidationException::withMessages([$field => __('service.not_found', ['attribute' => __('page.tax_rates')])]);
		}
		return $model;
	}
	/**
	...
	public function deleteMultiple($ids) {
		...
		$msg = trans_choice('service.multiple_deleted', count($ids), ['attribute' => __('page.tax_rates'), 'value' => count($ids)]);
		return $msg;
	}
