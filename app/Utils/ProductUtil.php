...
class ProductUtil extends Util {
	...
	public function productSerialNo($serial, $qty_remaining, $product_id, $variation_id) {

		$serial_no = array_filter(explode(',', trim($serial, ',')));
		$serial_no_count = count($serial_no);
		if ($serial_no_count != $qty_remaining) {
			throw ValidationException::withMessages(['message' => __('service.opening_stock_quantity_not_match')]);
		}

		foreach ($serial_no as $single_serial) {
			$product_serial = ProductSerial::where('serial_no', $single_serial)->first();

			if ($product_serial) {
				if ($product_serial->product_id == $product_id and $product_serial->variation_id == $variation_id) {
					if ($product_serial->sell_line_id) {
						throw ValidationException::withMessages(['message' => __('service.opening_stock_quantity_not_unique', ['serial' => $single_serial])]);
					}
				} else {
					throw ValidationException::withMessages(['message' => __('service.opening_stock_quantity_not_unique', ['serial' => $single_serial])]);
	...
	public function updateProductSerial($product_id, $variation_id, $serial_no, $purchase_line_id = Null) {

		//delete old serial
		if ($purchase_line_id) {
			ProductSerial::where('purchase_line_id', $purchase_line_id)->where('variation_id', $variation_id)->where('sell_line_id', Null)->delete();
		}
	...
	public function productSerialSold($serial_no, $sell_line_id) {
		if ($serial_no) {
			foreach ($serial_no as $value) {
				ProductSerial::where('serial_no', $value)->update(['sell_line_id' => $sell_line_id]);
			}
		}
	}
}
